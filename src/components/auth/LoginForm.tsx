...
const LoginForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Guardar token si existe y no hay uno ya en localStorage
      try {
      ...
      // Navegar a next o dashboard
      const params = new URLSearchParams(location.search)
      const next = params.get('next') || '/dashboard'
      navigate(next)
    } catch (error: any) {
      // Normalizar mensajes de error para que Cypress los encuentre
      const text = error?.message || 'Credenciales incorrectas'
      const normalized = ['Failed to fetch', 'Credenciales inválidas'].includes(text)
        ? 'Credenciales incorrectas'
        : text
      setMensaje(normalized)
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="mx-auto flex min-h-[80vh] w-full max-w-5xl flex-col items-center px-4 pb-16">
        ...
        <div className="mt-6 w-full max-w-xl rounded-2xl border border-slate-100 bg-white p-6 shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4 text-sm">
            ...
            {mensaje && (
              <p className="text-center text-xs font-medium text-red-500" role="alert" data-cy="login-alert">
                {mensaje}
              </p>
            )}
          </form>
