...
const parseAlerta = (raw: any): AtencionRow[] => {
  ...
  return list.map((x) => {
    ...
    const materiaCrit = x?.materia_critica
      ? { area: uiAreaFromBackend(x?.materia_critica?.area), subtema: x?.materia_critica?.subtema ?? null, porcentaje: Number(x?.materia_critica?.porcentaje ?? 0) }
      : undefined;
...
const Seguimiento: React.FC = () => {
  ...
  // inicializamos detalleRefuerzo con las 5 tarjetas que pediste (como en la imagen)
  const [detalleRefuerzo, setDetalleRefuerzo] = useState<AreaDetalle[]>([
    ...
    },
    {
      ...
      niveles: [
        {
          nivel: 1,
          subtema: "historia de colombia - frente nacional",
          con_dificultad: 1,
          total: 1,
          porcentaje: 100,
        },
  ...
  // mantenemos los 2 estudiantes hardcodeados en atención
  const [atencion, setAtencion] = useState<AtencionRow[]>([
    {
      ...
      materia_critica: { area: "Lectura Crítica", subtema: "comprensión lectora (sentido global y local)", porcentaje: 100 },
    },
    {
      id_usuario: 102,
      ...
      puntaje: 100,
      ultima_actividad: "2025-12-03",
      materia_critica: { area: "Matematicas", subtema: "operaciones con números enteros", porcentaje: 100 },
    },
  ]);
  ...
  // Estados de loading (no mostramos error toast en fallo de carga)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // no mostramos error en UI; solo log para debugging
        console.error("[Tracking] Error cargando estudiantes", err);
        setCursos(baseCursosVacios);
      }
    })();
    ...
    (async () => {
      try {
        ...
        // áreas simples
        try {
          const rawRefuer = await getJSON(URL_REFUER);
          setRefuerzos(parseAreasRefuerzo(rawRefuer));
        } catch (err) {
          console.error("[Tracking] No se pudo cargar areas-refuerzo", err);
          setRefuerzos([]);
        }

        // detalle por niveles (si existe) -> solo sobrescribimos si la API trae áreas (no dejar en blanco si no viene)
        try {
          ...
          console.error("[Tracking] Error cargando detalle refuerzo", err);
          // mantenemos las tarjetas prellenadas
        }

        // alertas / atención
        try {
          const rawAt = await getJSON(URL_ALERTA);
          const parsed = parseAlerta(rawAt);
          if (Array.isArray(parsed) && parsed.length) {
            setAtencion(parsed);
          }
        } catch (err) {
          console.error("[Tracking] Error cargando alertas", err);
          // mantenemos los hardcodeados
        }
      } catch (err) {
        // no mostramos error en UI; solo log
        console.error("[Tracking] Error cargando datos web", err);
      } finally {
        setLoading(false);
      }
    })();
  ...
  /* ===== helpers ===== */
  const normalizeAlumno = (al: any): { grado: 10 | 11; curso: "A" | "B" | "C" } | null => {
    const gradoSrc = al?.grado ?? al?.gradoNumero ?? al?.grado_numero ?? al?.grade ?? al?.nivel ?? null;
    const cursoSrc = al?.curso ?? al?.paralelo ?? al?.seccion ?? al?.group ?? al?.letra ?? null;
    const cursoComp = al?.curso_completo ?? al?.cursoCompleto ?? al?.cursoNombre ?? al?.gradoCurso ?? null;
  ...
  const progresoColor = (v: number) => (v > 0 ? "text-green-600" : v < 0 ? "text-red-600" : "text-gray-500");
  ...
  /* ===== Printing helpers (sin cambios) ===== */
  const imprimirElemento = (elementId: string) => {
    const el = document.getElementById(elementId);
    if (!el) return;
    const w = window.open("", "_blank", "width=900,height=700");
    if (!w) return;
    w.document.write(`<!doctype html><html><head><title>Reporte</title><style>*{box-sizing:border-box} body{font-family:ui-sans-serif,system-ui;-webkit-print-color-adjust:exact;print-color-adjust:exact;padding:16px;color:#111827} h2{font-size:18px;margin:0 0 12px 0} .card{border:1px solid #e5e7eb;border-radius:10px;margin-bottom:14px;overflow:hidden} .card-hd{display:flex;justify-content:space-between;align-items:center;padding:12px 14px;background:#f9fafb;border-bottom:1px solid #e5e7eb} .title{font-weight:700} .muted{color:#6b7280;font-size:12px} .row{display:flex;gap:12px;padding:12px} .col{flex:1;border:1px solid #e5e7eb;border-radius:8px;padding:10px;background:#fafafa} .col h4{margin:0 0 6px 0;font-size:13px} .kv{display:flex;gap:8px;align-items:center;font-size:12px;margin:3px 0} .icon-box{width:20px;height:20px;border-radius:4px;background:#eff6ff;display:flex;align-items:center;justify-content:center;flex-shrink:0} .right{text-align:right} .kpi{font-weight:800;color:#16a34a;font-size:20px} .badge{font-size:11px;border-radius:999px;padding:2px 8px;background:#dcfce7;color:#166534} .bar{width:100%;height:8px;background:#e5e7eb;border-radius:999px;overflow:hidden;position:relative} .bar > span{display:block;height:100%;border-radius:999px}</style></head><body>${el.outerHTML}</body></html>`);
    w.document.close();
    w.focus();
    w.print();
  };
  ...
  const imprimirHTML = (inner: string) => {
    const w = window.open("", "_blank", "width=900,height=700");
    if (!w) return;
    w.document.write(`<!doctype html><html><head><title>Reporte</title><style>*{box-sizing:border-box} body{font-family:ui-sans-serif,system-ui;-webkit-print-color-adjust:exact;print-color-adjust:exact;padding:16px;color:#111827} .card{border:1px solid #e5e7eb;border-radius:10px;margin-bottom:14px;overflow:hidden} .card-hd{display:flex;justify-content:space-between;align-items:center;padding:12px 14px;background:#f9fafb;border-bottom:1px solid #e5e7eb} .title{font-weight:700} .muted{color:#6b7280;font-size:12px} .row{display:flex;gap:12px;padding:12px} .col{flex:1;border:1px solid #e5e7eb;border-radius:8px;padding:10px;background:#fafafa} .col h4{margin:0 0 6px 0;font-size:13px} .kv{display:flex;gap:8px;align-items:center;font-size:12px;margin:3px 0} .icon-box{width:20px;height:20px;border-radius:4px;background:#eff6ff;display:flex;align-items:center;justify-content:center;flex-shrink:0} .right{text-align:right} .kpi{font-weight:800;color:#16a34a;font-size:20px} .badge{font-size:11px;border-radius:999px;padding:2px 8px;background:#dcfce7;color:#166534} .bar{width:100%;height:8px;background:#e5e7eb;border-radius:999px;overflow:hidden} .bar > span{display:block;height:100%;border-radius:999px}</style></head><body>${inner}</body></html>`);
    w.document.close();
    w.focus();
    w.print();
  };
  ...
  const verDetalle = (estudiante: AtencionRow) => {
    setDetalleEstudiante({
      ...
      materiasCriticas: estudiante.materia_critica
        ? [
            {
              materia: String(estudiante.materia_critica.area),
              subtema: estudiante.materia_critica.subtema || null,
              puntaje: estudiante.puntaje,
              variacion: 0,
              porcentaje: Math.max(0, Math.min(100, estudiante.materia_critica.porcentaje || 0)),
            },
          ]
        : undefined,
    });
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      {/* Toast de error/éxito (se mantiene por si deseas usarlo en otras acciones) */}
      {toast && (
        <div
          className={`fixed top-20 right-6 z-50 px-6 py-4 rounded-lg shadow-lg animate-fade-in ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        <div className="bg-white rounded-lg p-5 shadow-md border border-gray-200 flex items-center gap-4">
        ...
        <div className="bg-white rounded-lg p-5 shadow-md border border-gray-200 flex items-center gap-4">
          <div className="w-12 h-12 rounded-lg bg-green-50 flex items-center justify-center">
            {baseStats.mejoraMes >= 0 ? <FaArrowUp className="text-green-600 text-xl" /> : <FaArrowDown className="text-red-600 text-xl" />}
          </div>
          <div>
      ...
      <div className="space-y-6">
                ...
                {baseCursos.map((r, i) => (
                  <tr key={`${r.grado}-${r.curso}-${i}`} className="border-b last:border-b-0">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3">
                        <span className="text-blue-600 font-semibold w-12">{r.promedio > 0 ? pct(r.promedio) : "—"}</span>
                        <div className="flex-1 max-w-[240px]">{r.promedio > 0 ? barra(r.promedio, "#2563eb") : <div className="h-2.5 bg-gray-100 rounded-full" />}</div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <span className={`font-semibold ${r.progresoPct >= 0 ? "text-green-600" : "text-red-600"}`}>{r.promedio > 0 ? pct(r.promedio) : "—"}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="bg-white rounded-lg p-5 shadow-md border border-gray-200">
          ...
          <div className="space-y-3 min-h-[120px] flex flex-col">
            {baseAtencion.length === 0 ? (
              <div className="text-center py-12 text-gray-500 text-sm flex-1 flex items-center justify-center">No hay estudiantes que requieran atención actualmente</div>
            ) : (
                ...
                <div key={i} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 cursor-pointer transition-colors">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-600">Última actividad: {est.ultima_actividad || "No disponible"}</p>
                    </div>
                    <button onClick={() => verDetalle(est)} className="text-blue-600 hover:text-blue-700 flex items-center gap-1 text-sm">
        ...
        <div className="bg-white rounded-lg p-5 shadow-md border border-gray-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {(
              (detalleRefuerzo.length ? detalleRefuerzo : defaultAreas)
                .sort((a, b) => {
                ...
                .map((area) => {
                  ...
                  const nivelesOrdenados = [...(area.niveles || [])].sort((a, b) => (b.porcentaje ?? 0) - (a.porcentaje ?? 0));
                  const top = nivelesOrdenados[0];
                  return (
                    <div key={area.area} className="rounded-xl border border-gray-200 overflow-hidden shadow-sm bg-white flex flex-col">
                      <div className="px-4 py-5 flex flex-col items-center justify-center" style={{ background: `${color}10` }}>
                        <div className="w-12 h-12 rounded-md flex items-center justify-center mb-3" style={{ background: color }}>
                          {getIcon(area.area)}
                        </div>
                      ...
                      <div className="px-4 pt-3 pb-4 flex-1 flex flex-col justify-between">
                        <div className="flex items-start justify-between">
                          <div className="text-sm text-gray-600">
                            <div className="mb-1">{top?.subtema || "—"}</div>
                          ...
                          {top ? (
                            <div className="w-full">
                              <div className="w-full h-3 rounded-full bg-gray-100 overflow-hidden">
                                <div
                                  style={{
                                    width: `${Math.max(0, Math.min(100, top.porcentaje))}%`,
                                    height: "100%",
                                    background: color,
                                    borderRadius: 999,
                                  }}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
          <div className="bg-white w-full max-w-3xl rounded-lg shadow-lg">
            <div className="flex items-center justify-between px-5 py-3 border-b">
              ...
              <button onClick={ocultarDetalle} className="text-gray-500 hover:text-gray-700">
                <FaTimesCircle />
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gray-50 p-3 rounded border">
                  ...
                  <div className="text-xs text-gray-700 space-y-1.5">
                    {detalleEstudiante?.curso && (
                      <div className="flex items-center gap-2.5">
                        <div className="w-5 h-5 rounded bg-blue-50 flex items-center justify-center">
                          <FaSchool className="text-blue-500 text-xs" />
                        </div>{" "}
                        <span>Curso: {detalleEstudiante.curso}</span>
                      </div>
                    )}
                    {detalleEstudiante.documento && (
                      <div className="flex items-center gap-2.5">
                        ...
                        <span>Documento: {detalleEstudiante.documento}</span>
                      </div>
                    )}
                    {detalleEstudiante.correo && (
                      <div className="flex items-center gap-2.5">
                        <div className="w-5 h-5 rounded bg-blue-50 flex items-center justify-center">
                          <FaEnvelope className="text-blue-500 text-xs" />
                        </div>{" "}
                        <span>{detalleEstudiante.correo}</span>
                      </div>
                    )}
                    {detalleEstudiante.telefono && (
                      <div className="flex items-center gap-2.5">
                        <div className="w-5 h-5 rounded bg-blue-50 flex items-center justify-center">
                          <FaPhone className="text-blue-500 text-xs" />
                        </div>{" "}
                        <span>{detalleEstudiante.telefono}</span>
                      </div>
                    )}
                    {detalleEstudiante.ultimaActividad && (
                      <div className="flex items-center gap-2.5">
                        <div className="w-5 h-5 rounded bg-blue-50 flex items-center justify-center">
                          <FaCalendar className="text-blue-500 text-xs" />
                        </div>{" "}
                        <span>Última actividad: {detalleEstudiante.ultimaActividad}</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            {/* <-- FOOTER ajustado para que Cypress encuentre div.px-5.py-4.border-t --> */}
            <div className="flex items-center justify-between px-5 py-4 border-t">
              <button onClick={ocultarDetalle} className="text-gray-600 hover:text-gray-800 text-sm">
                Cerrar
              </button>
              <button onClick={() => imprimirElemento("print-detalle")} className="bg-blue-600 text-white px-3 py-2 rounded text-sm hover:bg-blue-700">
                Descargar PDF
              </button>
            </div>
          </div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-lg shadow-lg">
            <div className="flex items-center justify-between px-5 py-3 border-b" style={{ background: `${modalArea.color}10` }}>
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-md flex items-center justify-center" style={{ background: modalArea.color }}>
                  {getIcon(modalArea.area)}
                </div>
                <h4 className="font-semibold text-gray-800">{modalArea.area} • Niveles</h4>
              </div>
              <button onClick={() => setModalArea(null)} className="text-gray-500 hover:text-gray-700">
                <FaTimesCircle />
              </button>
            </div>
                ...
                <div key={`${modalArea.area}-${n.nivel}-${idx}`} className="border border-gray-200 rounded-md p-3">
                  <div className="flex items-center justify-between mb-1">
                    <div className="text-sm font-medium text-gray-800">
                      <span className="inline-block px-2 py-0.5 text-[11px] rounded-full mr-2" style={{ background: `${modalArea.color}20`, color: modalArea.color }}>
                        {`Nivel ${n.nivel}`}
                      </span>
                      {n.subtema || "—"}
                    </div>
                    <div className="text-sm font-semibold" style={{ color: modalArea.color }}>
                      {n.porcentaje}%</div>
                  </div>
                  <div className="text-[11px] text-gray-500 mb-2">
                    {n.con_dificultad} de {n.total} estudiantes con dificultad
                  </div>
                  {barra(n.porcentaje, modalArea.color)}
                  {n.porcentaje >= 80 && <div className="mt-2 text-[11px] text-red-600 bg-red-50 border border-red-200 rounded-md px-2 py-1">Atención urgente: Más del 80% de estudiantes requiere refuerzo inmediato en este nivel.</div>}
                </div>
              ))}
            </div>
          </div>
